...
class ZebraImage
{
	...
	{
		if (function_exists('imagefilter')) if ($this->_create_from_source()) {
			...
			if (is_array($filter)) {
				foreach ($filter as $arguments) if (defined('IMG_FILTER_' . strtoupper($arguments[0]))) {
	...
	private function _sharpen_image($image)
	{
		if ($this->sharpen_images && version_compare(PHP_VERSION, '5.1.0') >= 0) {
			$matrix = array(array(-1.2, -1, -1.2), array(-1, 20, -1), array(-1.2, -1, -1.2),);
