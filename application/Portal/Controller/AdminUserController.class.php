...
class AdminUserController extends AdminbaseController
{
	...
	function index()
	{
		...
		$users = $this->user_model->order(array("id" => "desc"))->limit($page->firstRow, $page->listRows)->select();
		foreach ($users as $key => $value) {
	...
	public function soUser()
	{
		...
		$page = $this->page($count, 20);
		$users = $this->user_model->where($where)->limit($page->firstRow, $page->listRows)->order($order)->select();
		foreach ($users as $key => $value) {
			$parent = $this->getParent($value['parent']);
			if ($parent) {
	...
	public function userroom()
	{
		...
		$room_list = M('user_room')->where(['uid' => $id])->order('id desc')->limit($room_page->firstRow, $room_page->listRows)->select();
	...
	protected function getChild($uid)
	{
		...
		$child = $this->user_model->where(array('parent' => $uid))->select();
		$child[] = $me;
		return $child;
	}
	...
	public function editMoney()
	{
		...
		$page = $this->page($count, 20);
		$record = $this->all_record->where($where)->order(array("id" => "desc"))->limit($page->firstRow, $page->listRows)->select();
	...
	public function activationCode()
	{
		...
		$page = $this->page($count, 20);
		$record = $this->card->order(array("id" => "desc"))->limit($page->firstRow, $page->listRows)->select();
		$this->assign('record', $record);
		$excel = $record;
	...
	public function soCode()
	{
		...
		$page = $this->page($count, 20);
		$record = $this->card->where($where)->order(array("id" => "desc"))->limit($page->firstRow, $page->listRows)->select();
		$this->assign('record', $record);
		$this->assign("page", $page->show('Admin'));
	...
	public function turntable()
	{
		if (IS_POST) {
			...
			$page = $this->page($count, 20);
			$record = $this->award->where($where)->order(array("id" => "desc"))->limit($page->firstRow, $page->listRows)->select();
			$this->assign('record', $record);
			$this->assign("page", $page->show('Admin'));
			$this->display();
			...
			$page = $this->page($count, 20);
			$res = $this->award->order(array("id" => "asc"))->limit($page->firstRow, $page->listRows)->select();
