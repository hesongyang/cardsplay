...
class ArticleController extends HomebaseController
{
	public function index()
	{
		...
		$posts_model = M("Posts");
		$article = $posts_model->alias("a")->field('a.*,c.user_login,c.user_nicename,b.term_id')->join("__TERM_RELATIONSHIPS__ b ON a.id = b.object_id")->join("__USERS__ c ON a.post_author = c.id")->where(array('a.id' => $article_id, 'b.term_id' => $term_id))->find();
		if (empty($article)) {
		...
		$next = $term_relationships_model->alias("a")->join($join)->join($join2)->where(array("post_date" => array("egt", $article_date), "object_id" => array('neq', $article_id), "a.status" => 1, 'a.term_id' => $term_id, 'post_status' => 1))->order("post_date asc")->find();
		$prev = $term_relationships_model->alias("a")->join($join)->join($join2)->where(array("post_date" => array("elt", $article_date), "object_id" => array('neq', $article_id), "a.status" => 1, 'a.term_id' => $term_id, 'post_status' => 1))->order("post_date desc")->find();
