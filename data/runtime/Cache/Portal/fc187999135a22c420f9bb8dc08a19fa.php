...
<head lang="zh-cn">
    ...
    <style>
        ...
        *,*:before,*:after {
        ...
        .top .switch .open-mask,.top .switch .open-member {
            position: absolute;
            left: 0;
            top: 0;
            right: 0;
        ...
        #border_3:checked + label:after {
            left: calc(100% - .2600rem);
        }

        #border_3 + label {
            border: .0096rem solid #ccc;
            -webkit-border-radius: .2400rem;
            -moz-border-radius: .2400rem;
            border-radius: .2400rem;
        ...
        .friend-item {
            ...
            height: .6336rem;
            align-items: center;
            border-bottom: .0096rem solid #ccc;
        }

        .friend-item .friend-content {
            padding: .0960rem;
        ...
        .friend-item .user-name-id {
            color: #000;
        }

        .friend-item .user-name {
            margin-right: .0240rem;
        }
        ...
        .friend-item .delete-mask2,.friend-item .button {
            position: absolute;
            top: 50%;
            right: .0960rem;
            margin-top: -.1440rem;
        ...
        .friend-item .delete-mask2 .delete,.friend-item .button .btn {
            width: .4800rem;
            height: .2880rem;
            line-height: .2880rem;
            text-align: center;
        ...
        .friend-item .delete-mask2 .delete {
...
<div class="wrap">
    <div class="top">
        ...
        <div class="switch">
            ...
            <div class="open-member"></div>
        </div>
    </div>
    <div class="user-friend"></div>
</div>
<?php if($user[sfgl] == 1): ?><script type="text/javascript">
    $('.top .switch .open-member').remove();
    $('#border_3').click();
</script>
<else>
    <script type="text/javascript">
        function guanli_no(){
            $.post("/index.php/portal/home/gbguanli",{zt:1},function(result){
                if(result.status=='1'){
                    ...
                    $('.grzxgl3').show();
                    alert2('关闭成功');
                    window.setTimeout("location.href ='/index.php/portal/dasheng/firendmanager'",1000);
                }
                else{
                    alert2(result.info);
                }
        ...
        $('.top .switch').click(function (e) {
            $.dialog("是否消耗100房卡关闭管理功能？", function() {
                guanli_no();
            });
            e.preventDefault();
        });
...
<script>
    ...
    var notice = {
        ...
        ws.connect = function(uri, fn) {return;
            var uri = uri || url;
            var fn = fn || func;
            if (uri == null) return;
            url = uri;
    ...
    var Page = {
        time: 0,
        wsSend: null,
        friendList: function(data, num, phone) {
            var getNew = num || 1;
            if (parseInt(phone) === 1) {
                for (var i in data) {
                    var code = '<div class="friend-item" data-id="' + data[i].id + '">';
                    code += '<div class="friend-content friend-content2 flex-cont" data-id="' + data[i].id + '" >';
                    code += '<div class="path">';
                    code += '<img src="' + data[i].path + '" alt="" onerror="this.src=\'../images/ucenter/user.png\'">';
                    code += '</div>';
                    code += '<div class="info flex-item">';
                    code += '<div class="user-name-id"><span class="user-name">' + data[i].nickname + '</span><span class="user-id">【ID:<em>' + data[i].id + '</em>】</span></div>';
                    code += '<div class="time">' + data[i].datetime + '</div>';
                    code += '</div>';
                    code += '<div class="button">';
                    if (data[i].status) {
                        if (parseInt(data[i].status) === 0) {
                            code += '<div class="btn" data-id="' + data[i].id + '">接受</div>';
                    ...
                    code += '</div>';
                    code += '</div>';
                    code += '<div class="delete-mask2">';
                    code += '<div class="delete" data-id="' + data[i].id + '">删除</div>';
                    code += '</div>';
                    code += '</div>';
                    if (parseInt(getNew) === 1) {
                        $(code).appendTo('.user-friend');
                    } else if (parseInt(getNew) === 2) {
                        $(code).prependTo('.user-friend');
                    }
                }
                $('.friend-item .btn').touch(function() {
                    var uid = $(this).data('id');
                    Page.wsSend(uid, 'acceptFriend');
                });
                $('.delete-mask2 .delete').touch(function() {
                    var id = $(this).data('id');
                    Page.wsSend(id, 'delFriend')
                });
                var initX, initY, moveX, moveY, X = 0,
                    Y = 0,
                    objX = 0,
                    flagX = 0;
                $('.friend-content').on('touchstart',
                    function(e) {
                        var that = $(this);
                        that.parent('.friend-item').siblings().children('.friend-content').css('left', 0);
                        that.css('left', 0);
                ...
                for (var i in data) {
                    var code = '<div class="friend-item" data-id="' + data[i].id + '">';
                    code += '<div class="friend-content flex-cont" data-id="' + data[i].id + '" >';
                    code += '<div class="path">';
                    code += '<img src="' + data[i].path + '" alt="" onerror="this.src=\'../images/ucenter/user.png\'">';
                    code += '</div>';
                    code += '<div class="info flex-item">';
                ...
                $('.friend-content').on('touchstart mousedown',
                    function(e) {
                        $(this).css({
                            backgroundColor: '#ccc',
                        });
                        $('.friend-content').on('contextmenu',
                            function(e) {
                                e.preventDefault();
                            });
                        var deleteid2 = $(this).data('id');
                        t = setTimeout(function() {
                                if ($('.delete-mask')) {
                                    $('.delete-mask').remove()
                                }
                                $('<div class="delete-mask">').html('<div class="delete" data-id="' + deleteid2 + '">删除</div>').appendTo('.wrap');
                                $('.delete-mask').touch(function() {
                                    $(this).remove();
                                });
                                $('.delete-mask .delete').touch(function() {
                                    var id = $(this).data('id');
                                    Page.wsSend(id, 'delFriend')
                                });
                                cancelTimeout();
                ...
                $(".friend-content").mouseup(function() {
                    $(this).css({
                        backgroundColor: '#fff',
                    });
                    cancelTimeout();
                });
                $(".friend-content").mouseout(function() {
                    $(this).css({
                        backgroundColor: '#fff',
                    });
                    cancelTimeout();
        ...
        load: function() {
            ...
            $('.user-friend').overscroll();
            $('#loadings').fadeOut('fast',
                function() {
                    $(this).remove();
                });
            $('.top .switch .open-member').on('click',
                function() {
                    $.dialog('是否消耗100张房卡开通好友管理功能？',
                        function() {
                            $.post("/index.php/portal/home/ktguanli",{zt:1},function(result){
                                if(result.status=='1'){
                                    ...
                                    $('.grzxgl3').hide();
                                    $.alert("开通成功！");
                                    window.setTimeout("location.href ='/index.php/Portal/index/index/skin/dasheng'",1000);
                                }
                                else{
                                    $.alert("房卡不足开通失败");
                                }
    ...
    wx.config({
        ...
        // 必填，公众号的唯一标识
        timestamp: '1512043865',
        // 必填，生成签名的时间戳
        nonceStr: 'crRcnC79w9qVrstw',
        // 必填，生成签名的随机串
        signature: 'abfffd3f96c44efbc5073874e918b959ff16400f',
        // 必填，签名，见附录1
